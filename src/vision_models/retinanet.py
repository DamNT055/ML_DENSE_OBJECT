...
class RetinaNet(nn.Module):
    ...
    @torch.jit.unused
    def eager_outputs(self, losses, detections, loss_only = False):
        # type: (Dict[str, Tensor], List[Dict[str, Tensor]], Any) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]
        if self.training or loss_only:
            return losses
    ...
    def forward(self, images, targets=None, loss_only = False):
        # type: (List[Tensor], Optional[List[Dict[str, Tensor]]], Any) -> Tuple[Dict[str, Tensor], List[Dict[str, Tensor]]]
        """
        Args:
        ...
        return self.eager_outputs(losses, detections, loss_only=loss_only)
